...
class Application(Frame):
    ...
    class DictionaryGUI(Toplevel):
        def __init__(this, dictionary, title = ""):
            ...
            Button(this,
                text = "Add",
                command = lambda : this.addSection(this.sectionName.get())
            ).grid(sticky = E+W)

            Button(this,
                text = "Add",
                command = lambda : this.addEntry(this.entryName.get())
            ).grid(row = 2, column = 3, sticky = E+W)

            Button(this,
                text = "Rename",
                command = lambda : this.renameSelectedSection(this.sectionName.get())
            ).grid(row = 2, column = 1, sticky = E+W)

            Button(this,
                text = "Rename",
                command = lambda : this.renameSelectedEntry(this.entryName.get())
